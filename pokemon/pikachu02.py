...
def main():
    ...
    for item in items.get("results"):
        ...
        if 'fire' in item.get("name"):
            # if TRUE, add that item to the end of list healwords
            healwords.append(item.get("name"))

    ## list all
    print(f"There are {len(healwords)} words that contain the word 'fire' in the Pokemon Item API!")
    print("List of Pokemon items containing fire: ")
    print(healwords)
